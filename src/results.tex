...
%When selecting the best optimisation sequence, they are ranked by the average of their performance measurement, using the work-based performance metric, except for the Oracle-RM which uses the real speedup over \texttt{-O0}.
\begin{itemize}
...
For example, the {\usefont{T1}{cmr}{m}{n} -loop-simplify} pass is used for transforming loops into a canonical form by inserting pre-header and exit basic blocks.
...
Because of the redundant basic blocks, this optimisation pass expects that the {\usefont{T1}{cmr}{m}{n} -simplifycfg} will eventually be executer later on the optimisation pipeline.
Another example of such inter-relation between transformations concerns the {\usefont{T1}{cmr}{m}{n} -licm} and {\usefont{T1}{cmr}{m}{n} -mem2reg} passes.
The {\usefont{T1}{cmr}{m}{n} -licm} pass is responsible for moving invariant code out from the loop body. 
It usually creates new local variables, using memory access operations, for assisting with the code manipulation, which means that the executing the {\usefont{T1}{cmr}{m}{n} -mem2reg} pass afterwards would be useful as a cleanup pass for removing the extra memory accesses generated.
However, many of the analysis required for identifying loop invariant also benefit from the transformations performed by the {\usefont{T1}{cmr}{m}{n} -mem2reg} pass.
...
Afterwards, this randomly generated optimisation would be included in the set of optimisations only if it was able to improve the performance of a training benchmark, also selected at random, in respect of the {\usefont{T1}{cmr}{m}{n} -O3} optimisation.
...
For some cases, namely for the tiff-related benchmarks, the work-based performance shows a worse correlation with speedup.
