...
In modern optimising compilers,  the correct choice of optimisation sequence can have a significant impact on the performance of the final optimised program.
...
Because of the unpredictability of optimisation interactions and their complex interaction,
...
In this work, we focus on enabling {\itercomp} in \textit{online} scenarios.
For this thesis, we define the online scenario as having the restriction that programs execute multiple inputs and distinct inputs are executed only once.
Because of these restrictions, standard metrics for comparing optimisations are not viable, such as speedups or execution time alone.
...
In order to reduce the profiling overhead, we propose two relaxation algorithms which provide a trade-off between measurement accuracy and overhead.

In the experimental evaluation, the first relaxation algorithm that operates on regions of functions is able to reduce the average overhead by about 43\% over the optimal work profiling, while incurring in a dynamic measurement error of much less than 1\%.
